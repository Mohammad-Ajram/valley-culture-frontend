...
import { getInvoice, updateInvoice, getProducts } from "../functions";
import { toast } from "react-toastify";
import { states, stateCodes, placeOfSupply } from "../utils/States";

const UpdateInvoice = ({ history, token }) => {
  ...
  useEffect(() => {
    getInvoice(token, id)
      .then((res) => {
        if (res.data.success) {
          setInvoiceProducts(res.data.invoice.products);
          setName(res.data.invoice.name);
          setEmail(res.data.invoice.email);
          setContact(res.data.invoice.contact);
          setInvoiceNo(res.data.invoice.invoiceNo);
          setLine1(res.data.invoice.address.line1);
          setLine2(res.data.invoice.address.line2);
          setCity(res.data.invoice.address.city);
          setState(res.data.invoice.address.state);
          setPincode(res.data.invoice.address.pincode);
          setDiscount(res.data.invoice.discount);
          setInvoiceValue(res.data.invoice.invoiceValue);
          setPayMethod(res.data.invoice.payMethod);
          setPayStatus(res.data.invoice.payStatus);
        }
      })
  ...
  return (
    <>
      <div className="d-flex align-items-center justify-content-between pr-5 pb-5">
        <h1 className="">Update Invoice</h1>
        <button
          type="button"
